...
namespace XNetCore.MyBatis.Common.Configuration
{
    ...
    public class MutableConfiguration : AbstractConfiguration
    {
        ...
        {
            const string ATTRIBUTE_NAMESPACE = "namespace";
            const string ATTRIBUTE_GLOBAL = "global::";
            if (this.internalId.ToLower().StartsWith(ATTRIBUTE_GLOBAL))
            {
                this.internalId = this.internalId.Substring(ATTRIBUTE_GLOBAL.Length);
            } else if (
                (name.Equals(ATTRIBUTE_NAMESPACE, StringComparison.CurrentCultureIgnoreCase))
                && (!this.internalId.StartsWith(value)))
            {
                this.internalId = value + "." + this.internalId;
            }
