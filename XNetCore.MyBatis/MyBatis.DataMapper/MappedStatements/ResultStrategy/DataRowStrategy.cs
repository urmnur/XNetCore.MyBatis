...
namespace XNetCore.MyBatis.DataMapper.MappedStatements.ResultStrategy
{
    ...
    public class DataRowStrategy : IResultStrategy
    {
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        public static string GetDataColumnName(string name, DataTable dt)
        {
            var result = name;
        ...
        public object Process(RequestScope request, ref IDataReader reader, object resultObject)
        {
            ...
            if (dataTable.Columns.Count==0)
            {
                ...
                if (resultMap is AutoResultMap)
                {
                    ...
                    {
                        string columnName = GetDataColumnName(reader.GetName(index), dataTable);
            ...
            if (resultMap is AutoResultMap)
            {
                for (int index = 0; index < reader.FieldCount; index++)
                {
                    string propertyName = dataRow.Table.Columns[index].ColumnName;//reader.GetName(index);
