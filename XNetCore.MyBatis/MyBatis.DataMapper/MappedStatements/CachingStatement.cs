...
namespace MyBatis.DataMapper.MappedStatements
{
    ...
    public sealed class CachingStatement : MappedStatementEventSupport, IMappedStatement
	{
        ...
        public DataSet ExecuteQueryForDataSet(ISession session, object parameterObject)
        {
            RequestRunner<DataSet> requestRunner = delegate(RequestScope requestscope, ISession session2, object parameter, CacheKey cachekey, out bool cachehit)
            {
                cachehit = true;
                DataSet dataSet = Statement.CacheModel[cachekey] as DataSet;
                if (dataSet == null)
                {
                    cachehit = false;
                    dataSet = mappedStatement.RunQueryForDataSet(requestscope, session, parameter);
                    Statement.CacheModel[cachekey] = dataSet;
                }

                return dataSet;
            };

            return CachingStatementExecute(PreSelectEventKey, PostSelectEventKey, session, parameterObject, "ExecuteQueryForDataSet", requestRunner);
        }
        ...
        /// <param name="parameterObject">The object used to set the parameters in the SQL.</param>
        /// <returns>The object</returns>
        public IDataReader ExecuteQueryForDataReader(ISession session, object parameterObject)
        {
            throw new NotSupportedException("IDataReader ���ܽ��л���");
        }
		/// <summary>
