...
namespace MyBatis.DataMapper.MappedStatements
{
    public partial class MappedStatement
    {
        ...
        public virtual DataSet ExecuteQueryForDataSet(ISession session, object parameterObject)
        {
            return Execute(PreSelectEventKey, PostSelectEventKey, session, parameterObject,
                (r, p) => RunQueryForDataSet(r, session, parameterObject));
        ...
        /// <returns></returns>
        internal DataSet RunQueryForDataSet(RequestScope request, ISession session, object parameterObject)
        {
                    ...
                    if (true) //isFirst
                    {
                        ...
                        for (int i = 0; i < reader.FieldCount; i++)
                        {
                            DataColumn col = new DataColumn(
                                        DataRowStrategy.GetDataColumnName(reader.GetName(i), dataTable),
                        ...
                        dataTable.TableName = System.Guid.NewGuid().ToString("D");
