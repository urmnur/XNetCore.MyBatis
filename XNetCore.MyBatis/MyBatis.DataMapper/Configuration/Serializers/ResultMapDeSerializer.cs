...
namespace XNetCore.MyBatis.DataMapper.Configuration.Serializers
{
    ...
    public sealed class ResultMapDeSerializer
    {
            ...
            )
        {
            ...
            string spaceName = ConfigurationUtils.GetMandatoryStringAttribute(config, ConfigConstants.ATTRIBUTE_NAMESPACE);
            if ((!string.IsNullOrEmpty(spaceName)) && (!className.StartsWith(spaceName)))
            {
                className = spaceName + "." + className;
            }
